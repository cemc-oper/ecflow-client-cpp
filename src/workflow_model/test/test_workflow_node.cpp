...
l.
path_ = "/meso_post/00/initial";
l.
variable_list_ = std::vector<NodeVariable>{
    NodeVariable{NodeVariableType::Variable, "VAR1", "var1_value"},
    NodeVariable{NodeVariableType::Variable, "VAR2", "var2_value"},
};

l.
generated_variable_list_ = std::vector<NodeVariable>{
...
EXPECT_EQ(j["variable_list"]
.

size(),

2);
EXPECT_EQ(j["variable_list"][0]["name"],
"VAR1");
EXPECT_EQ(j["generated_variable_list"]
.

size(),

3);
EXPECT_EQ(j["generated_variable_list"][2]["name"],
"GEN_VAR3");
}
...
node.
setNodeType(NodeType::Task);
node.

variableList()

.
path_ = "/meso_post/00/initail";
node.
...
inheritedVariableList()

.
push_back(hh_var_list);
...
.
push_back(suite_var_list);

auto j = node.toJson();

EXPECT_EQ(j["name"],
"initial");
EXPECT_EQ(j["variable_list"]["path"],
"/meso_post/00/initail");
EXPECT_EQ(j["variable_list"]["variable_list"]
.

size(),

2);
EXPECT_EQ(j["variable_list"]["variable_list"][0]["name"],
"MAX_COUNT");
EXPECT_EQ(j["inherited_variable_list"]
.

size(),

2);
EXPECT_EQ(j["inherited_variable_list"][1]["variable_list"][0]["name"],
"SUITE");
}
